...
public class GamePlayController {
	...
	public ResponseEntity<OngoingGamesOutput> getOngoingGamesController(@RequestHeader String user_oauth) throws Exception{
		System.out.println("GamePlayController - getOngoingGamesController - START - user_oauth >> " + user_oauth);
		...
		try {
			...
			System.out.println("GamePlayController - getOngoingGamesController - END - user_oauth >> " + user_oauth);
			return ResponseEntity.status(HttpStatus.OK).body(output);
		} catch (Exception e) {
			System.out.println("GamePlayController - getOngoingGamesController - ERROR - user_oauth >> " + user_oauth + " - exception >> " + e.getMessage());
	...
	public ResponseEntity<PlayWithBotOutput> playWithBotController(@RequestHeader String user_oauth, @RequestBody PlayWithBotInput input) throws Exception{
		System.out.println("GamePlayController - playWithBotController - START - user_oauth >> " + user_oauth + " - input >> " + input);
		
		PlayWithBotOutput output = null;
		try {
			output = service.playWithBotService(user_oauth, input);
			
			System.out.println("GamePlayController - playWithBotController - END - user_oauth >> " + user_oauth + " - input >> " + input);
			return ResponseEntity.status(HttpStatus.OK).body(output);			
		} catch (Exception e) {
			System.out.println("GamePlayController - playWithBotController - ERROR - user_oauth >> " + user_oauth + " - input >> " + input + " - exception >> " + e.getMessage());
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(output);
		}
